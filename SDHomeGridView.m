...
//        cycleScrollADViewBackgroundView.backgroundColor =COMMON_BACK_COLOR;
        [self addSubview:cycleScrollADViewBackgroundView];
        _cycleScrollADViewBackgroundView = cycleScrollADViewBackgroundView;
        borderColor=[UIColor colorWithWhite:0.95 alpha:1.0];
...
- (void)setGridModelsArray:(NSArray *)gridModelsArray
{
    ...
    [gridModelsArray enumerateObjectsUsingBlock:^(SDHomeGridItemModel *model, NSUInteger idx, BOOL *stop) {
        ...
        item.backgroundColor=idx%2==0?[UIColor colorWithWhite:0.984 alpha:1.000]:[UIColor whiteColor];
...
- (void)saveItemsSettingCache
{
    ...
    if ([self.gridViewDelegate respondsToSelector:@selector(homeGrideViewDidChangeItems:newArr:)]) {
        NSMutableArray *arr=[NSMutableArray new];
        for(SDHomeGridViewListItemView *item in _itemsArray){
            [arr addObject:item.itemModel];
        }
        [self.gridViewDelegate homeGrideViewDidChangeItems:self newArr:[arr copy]];
    }
}
...
- (void)setupSubViewsFrame
{
    ...
    [_itemsArray enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop) {
        ...
        UIView *v=[[UIView alloc] initWithFrame:CGRectMake(itemW-borderWidth, 0, borderWidth, itemH)];
        v.backgroundColor=borderColor;
        [item addSubview:v];
        
        UIView *v1=[[UIView alloc] initWithFrame:CGRectMake(0, itemH-borderWidth, itemW, borderWidth)];
        v1.backgroundColor=borderColor;
        [item addSubview:v1];
    }];
}
