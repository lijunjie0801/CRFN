...
//        [_selectedBtn setImage:[UIImage imageNamed:@"打钩"] forState:UIControlStateSelected];
        ...
        UITapGestureRecognizer *tag = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(HeadClick)];
...
- (void)selectedClick:(UIButton *)sender{
    ...
    if (sender.selected) {
        ...
        [_selectedBtn setImage:[UIImage imageNamed:@"打钩"] forState:UIControlStateNormal];
    ...
    if ([self.delegate respondsToSelector:@selector(shopCartHeadViewCell:withSelectedStore:)]) {
        [self.delegate shopCartHeadViewCell:cell withSelectedStore:cell.shopCarModel.sellerId];
    }
}
...
-(void)HeadClick
{
    ...
    ShopCartHeadViewCell *cell = (ShopCartHeadViewCell *)_selectedBtn.superview.superview;
    if ([self.delegate respondsToSelector:@selector(shopCartHeadViewCell:withSelectedStore:)]) {
        [self.delegate shopCartHeadViewCell:cell withSelectedStore:cell.shopCarModel.sellerId];
    }
}
...
//    NSLog(@"==================%d",shopCarModel.isSelect);
//    _selectedBtn.selected = shopCarModel.isSelect;
    ...
    _shopNameLabel.text = shopCarModel.companName;
